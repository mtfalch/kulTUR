...
def tracks(location):
	...
	result = db.session.query(func.ST_AsGeoJSON(Tracks.geog)).filter_by(rutenavn=location).all();
	result = [loads(r[0]) for r in result]
	result = dict(type='FeatureCollection', features=result)
